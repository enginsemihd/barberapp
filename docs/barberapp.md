...
- **SMS/WhatsApp**: Twilio (adapter pattern arkasında — ileride değiştirilebilir); onay mesajı randevu oluşturulduğu an, hatırlatmalar 24s/2s önce
...
- **Onay:** Müşteri randevuyu onayladığı an bir Edge Function tetiklenir → randevu `confirmed` olur → müşterinin tercih ettiği kanaldan (WhatsApp/SMS) anlık onay mesajı gönderilir → `notifications_log`'a `confirmation` olarak yazılır
- **Hatırlatma:** pg_cron her birkaç dakikada bir çalışan bir Edge Function tetikler → yaklaşan (24s/2s) randevuları bulur → Twilio ile aynı tercih edilen kanaldan gönderir → `notifications_log`'a `reminder_24h`/`reminder_2h` olarak yazılır
...
6. **Bildirimler:** Twilio entegrasyonu, anlık onay mesajı, pg_cron hatırlatma job'u
